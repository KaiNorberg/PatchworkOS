 ...
 */
static bool basic_allocator_alloc_pages(pfn_t* pfns, size_t amount)
{
...
/**
 * @brief Loads the initrd.tar RAM disk from the boot volume.
 *
 * @param initrd Pointer to the initrd structure to populate.
 * @param rootHandle Handle to the root of the boot volume.
 * @return EFI_SUCCESS on success, error code otherwise.
 */
static EFI_STATUS initrd_load(boot_initrd_t* initrd, EFI_FILE* rootHandle)
{
    if (initrd == NULL || rootHandle == NULL)
    {
        return EFI_INVALID_PARAMETER;
    }
    ...
    status = uefi_call_wrapper(bootDir->Open, 5, bootDir, &fileHandle, L"initrd.tar", EFI_FILE_MODE_READ,
        EFI_FILE_READ_ONLY | EFI_FILE_HIDDEN | EFI_FILE_SYSTEM);
    uefi_call_wrapper(bootDir->Close, 1, bootDir);
    if (EFI_ERROR(status))
    {
        Print(L"  ERROR: Failed to open initrd.tar\n");
        return status;
    }
    ...
    initrd->buffer = AllocatePool(initrd->size);
    if (initrd->buffer == NULL)
    {
        uefi_call_wrapper(fileHandle->Close, 1, fileHandle);
        return EFI_OUT_OF_RESOURCES;
    }

    UINTN readSize = initrd->size;
    status = uefi_call_wrapper(fileHandle->Read, 3, fileHandle, &readSize, initrd->buffer);
    if (EFI_ERROR(status) || readSize != initrd->size)
    {
        FreePool(initrd->buffer);
        initrd->buffer = NULL;
        uefi_call_wrapper(fileHandle->Close, 1, fileHandle);
        return EFI_LOAD_ERROR;
    }
...
static EFI_STATUS boot_info_populate(EFI_HANDLE imageHandle, EFI_SYSTEM_TABLE* systemTable, boot_info_t* bootInfo)
{
    ...
    Print(L"Initializing initrd...\n");
    status = initrd_load(&bootInfo->initrd, rootHandle);
    if (EFI_ERROR(status))
    {
