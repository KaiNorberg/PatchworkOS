 ...
 *
 * Each syncronization control object is specified via an address within the `SYS_SYNC_CTL` system call, this address will be translated into its physical address to actually access the object. This means that if the specified address is within shared memory both processes will be able to synchronize on the same object. 
 * 
