...
static uint64_t element_send_init(element_t* elem)
{
    ...
    if (elem->proc(elem->win, elem, &event) == PFAIL)
    {
        return PFAIL;
    }
    ...
    procedure_t procedure, void* data)
{
    ...
    if (element_send_init(elem) == PFAIL)
    {
        element_free(elem);
        return NULL;
    }
...
uint64_t element_dispatch(element_t* elem, const event_t* event)
{
    ...
    case EVENT_MOUSE:
    {
        ...
        if (elem->proc(elem->win, elem, &movedEvent) == PFAIL)
        {
            return PFAIL;
        }
        ...
        LIST_FOR_EACH(child, &elem->children, entry)
        {
            if (element_dispatch(child, &movedEvent) == PFAIL)
            {
                return PFAIL;
            }
        }
    ...
    default:
    {
        ...
        LIST_FOR_EACH(child, &elem->children, entry)
        {
            if (element_dispatch(child, event) == PFAIL)
            {
                return PFAIL;
            }
        }
