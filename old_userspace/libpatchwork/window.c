...
static uint64_t window_deco_init(window_t* win, element_t* elem)
{
  ...
  private
    ...
    if (!(win->flags & WINDOW_NO_CONTROLS))
    {
        if (window_deco_init_controls(win, elem, private) == PFAIL)
        {
            free(private);
            return PFAIL;
        }
    ...
    procedure_t procedure, void* data)
{
    ...
    event_t event;
    if (display_wait(disp, &event, EVENT_SURFACE_NEW) == PFAIL)
    {
        window_free(win);
        return NULL;
    }
    ...
    fd_t shmem;
    status_t status = claim(&shmem, event.surfaceNew.shmemKey);
    if (IS_ERR(status))
    {
        window_free(win);
        return NULL;
    ...
    win->buffer = NULL;
    status = iomap(shmem, (void**)&win->buffer, RECT_WIDTH(&win->rect) * RECT_HEIGHT(&win->rect) * sizeof(pixel_t), 0,
...
uint64_t window_dispatch(window_t* win, const event_t* event)
{
    ...
    case EVENT_LIB_REDRAW:
    {
        ...
        if (element_dispatch(elem, event) == PFAIL)
        {
            return PFAIL;
        }
    }
    ...
    case EVENT_REPORT:
    {
        ...
        if (element_dispatch(win->root, event) == PFAIL)
        {
            return PFAIL;
        }
    }
...
uint64_t window_set_visible(window_t* win, bool isVisible)
{
    ...
    if (display_dispatch_pending(win->disp, EVENT_LIB_REDRAW, win->surface) == PFAIL)
    {
        return PFAIL;
    }
