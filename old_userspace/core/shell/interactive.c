...
{
    char cwd[MAX_PATH] = {0};
    if (IS_ERR(readfile("/proc/self/cwd", cwd, MAX_PATH - 1, 0, NULL)))
    {
        strcpy(cwd, "?");
    }
    printf("\n\033[32m%s\n\033[92m>\033[m ", cwd);
...
static void interactive_execute(void)
{
    ...
    pipeline_t pipeline;
    if (IS_ERR(pipeline_init(&pipeline, buffer, STDIN_FILENO, STDOUT_FILENO, STDERR_FILENO)))
    {
        interactive_prompt();
        return;
    }
...
static void interactive_handle_input(const char* input, uint64_t length)
{
    for (uint64_t i = 0; i < length; i++)
    {
        ...
        if (ansi_parse(&ansi, input[i], &result) == PFAIL)
        {
            continue;
        }
...
void interactive_shell(void)
{
    ...
    {
        proc_exit(IOFMT("shell: failed to set SIGINT handler (%s)\n", strerror(errno)));
    }
    ...
    while (true)
    {
        ...
        size_t readCount;
        status_t status = RETRY_ON_CODE(ioread(STDIN_FILENO, buffer, MAX_PATH, IOCUR, &readCount), INTR);
        if (IS_ERR(status))
        {
