...
static list_t sockets = LIST_CREATE(sockets);
...
static void socket_free(socket_t* socket)
{
    ...
    if (list_contains_entry(&sockets, &socket->listEntry))
    {
        list_remove(&sockets, &socket->listEntry);
    }
...
{
    net_family_t* family = file->path.dentry->parent->inode->private;
    assert(family != NULL);
    net_family_file_t* familyFile = file->path.dentry->inode->private;
    assert(familyFile != NULL);
    dentry_t* root = file->inode->superblock->root;
    assert(root != NULL);

    socket_t* socket = socket_new(family, familyFile->type);
    if (socket == NULL)
    {
        return ERR;
    }
    ...
    rwlock_write_acquire(&lock);
    list_push_back(&sockets, &socket->listEntry);
    rwlock_write_release(&lock);
...
static uint64_t netfs_addrs_read(file_t* file, void* buffer, uint64_t count, uint64_t* offset)
{
    ...
    char* string = malloc(list_length(&sockets) * (MAX_PATH + 1));
    if (string == NULL)
    {
        errno = ENOMEM;
        return ERR;
    ...
    uint64_t length = 0;
    socket_t* socket;
    LIST_FOR_EACH(socket, &sockets, listEntry)
    {
...
static uint64_t netfs_family_lookup(inode_t* dir, dentry_t* dentry)
{
    for (size_t i = 0; i < ARRAY_SIZE(familyFiles); i++)
    {
        ...
        inode_t* inode = inode_new(dir->superblock, ino_gen(dir->number, familyFiles[i].name), INODE_FILE, NULL,
            familyFiles[i].fileOps);
        if (inode == NULL)
        {
            return ERR;
        }
        UNREF_DEFER(inode);
        inode->private = &familyFiles[i];
