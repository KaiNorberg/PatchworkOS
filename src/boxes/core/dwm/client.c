...
static uint64_t client_action_surface_new(client_t* client, const cmd_header_t* header)
{
    ...
    if (IS_ERR(share(event.shmemKey, sizeof(event.shmemKey), surface->shmem, CLOCKS_NEVER)))
    {
        surface_free(surface);
        return PFAIL;
    }

    if (dwm_attach(surface) == PFAIL)
    {
        surface_free(surface);
        return PFAIL;
    }
...
static uint64_t client_process_cmds(client_t* client, cmd_buffer_t* cmds)
{
    ...
    CMD_BUFFER_FOR_EACH(cmds, cmd)
    {
        if (actions[cmd->type](client, cmd) == PFAIL)
        {
...
uint64_t client_receive_cmds(client_t* client)
{
    ...
    size_t readSize;
    status_t status = ioread(client->fd, client->recvBuffer + client->recvLen, freeSpace, IOCUR, &readSize);
    if (IS_ERR(status))
    {
    ...
    while (client->recvLen > 0)
    {
        ...
        if (client_process_cmds(client, cmds) == PFAIL)
        {
            return PFAIL;
        }
...
static uint64_t client_send_all(fd_t fd, const void* data, size_t size)
{
    ...
    {
        size_t n;
        status_t status = iowrite(fd, p + sent, size - sent, IOCUR, &n);
        if (IS_ERR(status))
        {
            if (IS_CODE(status, AGAIN))
            {
...
uint64_t client_send_event(client_t* client, surface_id_t target, event_type_t type, void* data, uint64_t size)
{
    if (client->bitmask[type / 64] & (1ULL << (type % 64)))
    {
        ...
        if (client_send_all(client->fd, &event, sizeof(event_t)) == PFAIL)
        {
            return PFAIL;
        }
    }
