...
static uint64_t hpet_init(sdt_header_t* table)
{
    ...
    LOG_INFO("started HPET timer phys=0x%016lx virt=0x%016lx period=%llufs (%lluns) timers=%u %s-bit\n", hpet->address,
        address, period, period / 1000000, hpet->comparatorCount + 1, hpet->counterIs64Bit ? "64" : "32");
