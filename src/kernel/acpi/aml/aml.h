 ...
 * system. A hardware manufacturer creates this bytecode to describe their hardware, we, as the kernel, parse this
 * bytecode, the bytecode contains instructions that create namespaces and provide device information. But it does not
 * output this data, its not like JSON or similar, instead AML itself expects a series of functions (e.g., for creating
 * device nodes, namespaces, etc.) that it can call to directly create these structures.
 *
 ...
 * the `aml_parse_termlist()` function. A termlist is defined as `TermList := Nothing | <termobj termlist>` so then we
 * call the `aml_parse_termobj()` function. A termobj is defined as `TermObj := Object | StatementOpcode |
 * ExpressionOpcode` we then determine which this specific termobj is and continue down the chain until we finally have
