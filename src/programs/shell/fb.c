...
static uint8_t glyphBuffer[16 * 512];
...
static void fb_load_font()
{
	...
	{
	    bufferSize = header.charSize * 256;
	}
    
    seek(fd, sizeof(PsfHeader), SEEK_SET);
    read(fd, glyphBuffer, bufferSize);
...
{
    offset *= fbInfo.pixelsPerScanline;
    memcpy(FB_ADDR, (void*)(((uint64_t)FB_ADDR) + offset), fbInfo.size - offset);
...
void fb_char(char chr, uint64_t x, uint64_t y, uint32_t foreground, uint32_t background)
{
    ...
    for (uint32_t yOffset = 0; yOffset < FB_CHAR_HEIGHT; yOffset++)
    {
        for (uint32_t xOffset = 0; xOffset < FB_CHAR_WIDTH; xOffset++)
        {                
            uint32_t pixel = (*(glyph + yOffset) & (0b10000000 >> xOffset)) > 0 ? foreground : background;

            *((uint32_t*)(FB_ADDR + (x + xOffset) * sizeof(uint32_t) + 
            (y + yOffset) * fbInfo.pixelsPerScanline * sizeof(uint32_t))) = pixel;
        }
    }
}
