 ...
 *
 * The kernel and already loaded kernel modules are able to announce changes in the state of devices. This is done by calling `announce_device()` which will queue a string to be read by user space in the `/dev/announce` file described below.
 * 
 * ## /dev/announce
 * 
 * The `/dev/announce` file is used to announce device state changes to user space. When a device is attached or detached, a message is written to this file. User space daemons can read this file to react to hardware changes, such as loading the appropriate drivers.
 * 
 ...
 * 
 * Note that in the future a message might have additional fields, as such the `\n` character should always be used to find the end of a message. The order and meaning of existing fields will remain unchanged.
 * 
 * @note The compat device type is to allow for generic driver loading. As an example, we might have a driver for the exact device being announced, in which case the normal device type is enough. However, even if we dont have a driver for that exact device, a generic driver for similar devices may suffice, this is the compat device type.
 * 
 * @{
 */
