 ...
 * @defgroup libc_common_heap Heap
 * @ingroup libc_common
 *
 ...
 * mapped directly.
 *
 * To allow for coalescing of free blocks, all blocks (allocated and free) are additionally stored in a linked list
 * sorted by address. When a block is freed, we check the previous and next blocks in memory to see if they are free,
 * and if so we merge them into a single larger block.
 *
